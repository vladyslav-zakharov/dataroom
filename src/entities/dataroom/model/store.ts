...
export const useDataroomStore = create<DataroomStore>()((set, get) => ({
  ...
  renameDataroom: (id: string, newName: string): void => {
    ...
    const next = datarooms.map((d) =>
      d.id === id ? { ...d, name: trimmed, updatedAt: nowIso() } : d,
    )
    set({ datarooms: next })
    void persistDatarooms(next)
  },
  ...
  renameNode: (id: string, newName: string): void => {
    ...
    const siblings = siblingNames(
      nodes,
      { dataroomId: node.dataroomId, parentId: node.parentId },
      id,
    )
    if (siblings.has(trimmed.toLowerCase())) {
      throw new DataroomError(
    ...
    const next = nodes.map((n) =>
      n.id === id ? { ...n, name: trimmed, updatedAt: nowIso() } : n,
    )
    set({ nodes: next })
    void persistNodes(next)
  },
  ...
  getChildren: (dataroomId: string, parentId: string | null): DataroomNode[] =>
    getChildren(get().nodes, dataroomId, parentId),

  getPath: (nodeId: string): DataroomNode[] => getPath(get().nodes, nodeId),

  getDescendantCounts: (nodeId: string): { folders: number; files: number } =>
    getDescendantCounts(get().nodes, nodeId),
