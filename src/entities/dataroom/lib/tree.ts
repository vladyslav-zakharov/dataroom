 ...
 */
import type { DataroomNode, FileNode, FolderNode } from '../model/types'
 ...
 */
export const resolveUploadName = (desired: string, existingSiblingNames: Set<string>): string => {
  if (!existingSiblingNames.has(desired.toLowerCase())) {
    return desired
  }
...
export const getPath = (nodes: DataroomNode[], nodeId: string): DataroomNode[] => {
...
export const getDescendantIds = (nodes: DataroomNode[], nodeId: string): string[] => {
  const result: string[] = []
...
export const getDescendantCounts = (
  nodes: DataroomNode[],
  nodeId: string,
 ...
 *
 * @param nodes - full flat node list
 * @param scope - { dataroomId, parentId } context to filter siblings
 * @param excludeId - optional node id to exclude (used during rename)
 */
export const siblingNames = (
  nodes: DataroomNode[],
  scope: { dataroomId: string; parentId: string | null },
  excludeId?: string,
): Set<string> =>
  new Set(
    nodes
